...
bot = Bot(
    ...
    port=8000,
    webhook='https://1e16685a.ngrok.io',  # Webhook url
)
...
async def nnum(chat: Chat, matched):
    text = '''Друзья! С радостью сообщаем Вам о  системе лояльности в Pizza&Coffee! С ее помощью вы гарантированно получаете кэшбэк 10% с каждого заказа! Чтобы вы могли досконально во всем разобраться, мы подготовили для вас руководство в формате вопрос-ответ.'''
...
async def nnum(chat: Chat, matched):
    text = '''С ней вы сэкономите свои средства, а также получите многие другие бонусы, доступные только владельцам карт, которые сделают каждое посещение Pizza&Coffee еще более приятным и привычным занятием.'''
    await chat.send_text(text, keyboard=Keyboard(kb.card, bg_color="#FFFFFF"))


@bot.command('КАК ОНА РАБОТАЕТ')
async def nnum(chat: Chat, matched):
    text = '''С каждого заказа, который вы сделаете, на карту будет начислено 10% от его суммы в виде баллов.Эти средства вы можете накапливать и использовать при оплате следующих заказов.Например, вы купили большую 43-сантиметровую пиццу «4 сезона» за 22,90 руб. После оплаты этой суммы, на карте окажется 2,29 рублей (10%) и в следующий раз вы сможете их использовать и сэкономить.А можете просто продолжать накапливать баллы. Обратите внимание, что при оформлении одного заказа вы можете либо потратить свои баллы, либо накопить их, но не одновременно!Также обладатели клубных карт имеют уникальную возможность принимать участие в акциях, проводимых только для обладателей клубных карт!'''
    await chat.send_text(text, keyboard=Keyboard(kb.card, bg_color="#FFFFFF"))


@bot.command('КАКОВА МАКСИМАЛЬНАЯ СКИДКА, КОТОРУЮ ВЫ МОЖЕТЕ ПОЛУЧИТЬ С ПОМОЩЬЮ КАРТЫ')
async def nnum(chat: Chat, matched):
    text = '''99,9%. На примере с той же большой пиццей «4 сезона», вы сможете приобрести ее за 23 копейки, если у вас на карте будет накоплено 22,88 балла. Если баллов больше, то оставшиеся не сгорят, а останутся доступными для будущих заказов.'''
    await chat.send_text(text, keyboard=Keyboard(kb.card, bg_color="#FFFFFF"))
